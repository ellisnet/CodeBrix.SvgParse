...
namespace CodeBrix.SvgParse.Generators //Was previously: namespace Svg.Generators
{
    /// <summary>
    /// Generates available elements ElementInfo metadata for SvgElementFactory.
    /// </summary>
#if ENABLE_INCREMENTAL_GENERATOR
    [Generator]
    public partial class AvailableElementsGenerator : IIncrementalGenerator
    {
...
#pragma warning restore RS2008 // Enable analyzer release tracking
        ...
        private void Execute(SourceProductionContext context, Compilation compilation, System.Collections.Immutable.ImmutableArray<ClassDeclarationSyntax> candidateClasses)
        {
            var elementFactoryAttribute = compilation.GetTypeByMetadataName("CodeBrix.SvgParse.ElementFactoryAttribute");
            ...
            var svgElementBaseSymbol = compilation.GetTypeByMetadataName("CodeBrix.SvgParse.SvgElement");
            ...
            foreach (var elementFactorySymbol in elementFactorySymbols)
            {
                try
                {
                    ProcessClass(
                        ...
                        (name, text) => context.AddSource(name, text),
                        message => context.ReportDiagnostic(Diagnostic.Create(ErrorDescriptor, Location.None, message)));
                }
                catch (Exception ex)
                {
...
#else
    [Generator]
#pragma warning disable RS1035 // Non-incremental source generator
    public partial class AvailableElementsGenerator : ISourceGenerator
    {
        private static readonly DiagnosticDescriptor ErrorDescriptor = new DiagnosticDescriptor(
#pragma warning disable RS2008 // Enable analyzer release tracking
            "SV0000",
...
#pragma warning restore RS1035
    ...
    // Shared generation logic used by both incremental and non-incremental paths.
    public partial class AvailableElementsGenerator
    {
        /// <summary>
        ...
        /// <param name="addSource">Action to add generated source to the output.</param>
        /// <param name="reportError">Action to report a diagnostic error.</param>
        private static void ProcessClass(Compilation compilation, INamedTypeSymbol elementFactorySymbol, List<INamedTypeSymbol> svgElementSymbols, INamedTypeSymbol svgElementBaseSymbol, Action<string, SourceText> addSource, Action<string> reportError)
        {
            ...
            {
                reportError("Invalid ElementFactory symbol namespace.");
                return;
            }

            // Get SvgElementAttribute symbol using for later attribute retrieval.
            var svgElementAttribute = compilation.GetTypeByMetadataName("CodeBrix.SvgParse.SvgElementAttribute");
            if (svgElementAttribute is null)
            {
                reportError("Could not get Svg.SvgElementAttribute metadata.");
                return;
            }

            // Get SvgAttributeAttribute symbol using for later attribute retrieval.
            var svgAttributeAttribute = compilation.GetTypeByMetadataName("CodeBrix.SvgParse.SvgAttributeAttribute");
            if (svgAttributeAttribute is null)
            {
                reportError("Could not get Svg.SvgAttributeAttribute metadata.");
                return;
            }
...
");
            addSource($"CodeBrix.SvgParse.SvgElement.Properties.g.cs", SourceText.From(source.ToString(), Encoding.UTF8));
            source.Clear();
...
");
                addSource($"{namespaceElement}.{element.Symbol.Name}.Properties.g.cs", SourceText.From(source.ToString(), Encoding.UTF8));
                source.Clear();
            }
...
");
            addSource($"CodeBrix.SvgParse.SvgElements.g.cs", SourceText.From(source.ToString(), Encoding.UTF8));
            source.Clear();
...
");
            ...
            addSource($"{namespaceElementFactory}.{elementFactorySymbol.Name}.ElementFactory.g.cs", SourceText.From(source.ToString(), Encoding.UTF8));
            source.Clear();
        }
...
");
        ...
        /// <summary>
        /// Get the TypeConverter type string set for property/event symbol or property/event symbol type.
        /// </summary>
        ...
        /// <summary>
        /// Get the TypeConverter type string set as attribute on symbol.
        /// </summary>
