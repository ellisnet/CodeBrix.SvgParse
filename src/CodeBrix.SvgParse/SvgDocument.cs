...
public partial class SvgDocument : SvgFragment, ITypeDescriptorContext
{
    ...
    public static ExternalType ResolveExternalXmlEntities { get; set; } = ExternalType.None;
    ...
    /// <summary>
    /// Library-wide default for named-color serialization. When <c>true</c>, colors that match
    /// a W3C/CSS3 named color (e.g. <c>#ff0000</c>) are written out using the name
    /// (<c>fill="red"</c>). When <c>false</c> (the default), all colors are written as hex.
    /// <para>
    /// This value is snapshotted into a new <see cref="SvgDocument"/>'s
    /// <see cref="EnableEmitNamedColorsOnSerialization"/> property at construction time.
    /// Changing this static later does not update existing documents.
    /// </para>
    /// </summary>
    public static bool EmitNamedColorsOnSerialization { get; set; } = false;

    private static int? pointsPerInch;

    /// <summary>Gets or sets the PointsPerInch.</summary>
    ...
    /// </summary>
    /// <summary>
    /// Per-document override for named-color serialization. Initialized from the
    /// library-wide <see cref="EmitNamedColorsOnSerialization"/> value at construction
    /// time. Later changes to the static property do not propagate to existing documents.
    /// </summary>
    public bool EnableEmitNamedColorsOnSerialization { get; set; }

    /// <summary>Gets or sets the SvgDocument().</summary>
    /// <summary>Initializes a new instance of the <see cref="SvgDocument"/> class.</summary>
    public SvgDocument()
    {
        Ppi = PointsPerInch;
        EnableEmitNamedColorsOnSerialization = EmitNamedColorsOnSerialization;
    ...
    public virtual void RasterizeDimensions(ref SvgSizeF size, int rasterWidth, int rasterHeight)
    {
        if (size.Width == 0)
            return;
    ...
    /// <summary>Gets or sets the Write(XmlWriter).</summary>
    /// <inheritdoc />
    ...
    /// <summary>Gets or sets the Write(Stream, bool).</summary>
    /// <summary>Performs the Write(Stream, bool) operation.</summary>
    ...
    /// <summary>Gets or sets the Write(string, bool).</summary>
    /// <summary>Performs the Write(string, bool) operation.</summary>
