...
public partial class SvgDeferredPaintServer : SvgPaintServer
{
    ...
    // TODO: In the future, consider refactoring deferred paint server resolution so that
    // the source document is threaded through the resolution pipeline (e.g. via EnsureServer)
    // rather than stored directly on each paint server instance. This would eliminate the
    // need to carry and clone a Document reference on every SvgDeferredPaintServer.
    /// <summary>Gets or sets the Document.</summary>
    public SvgDocument Document { get; set; }

    /// <summary>Gets or sets the DeferredId.</summary>
    public string DeferredId { get; set; }

    /// <summary>Gets or sets the FallbackServer.</summary>
    public SvgPaintServer FallbackServer { get; private set; }

    /// <summary>Gets or sets the SvgDeferredPaintServer().</summary>
    /// <summary>Initializes a new instance of the <see cref="SvgDeferredPaintServer"/> class.</summary>
    public SvgDeferredPaintServer()
    {
    }

    // TODO: In the future, consider consolidating this constructor with the
    // (SvgDocument, string, SvgPaintServer) overload so there is a single constructor
    // path that captures the source document and supports fallback servers.
    /// <summary>Gets or sets the SvgDeferredPaintServer(SvgDocument, string).</summary>
    /// <summary>Initializes a new instance of the <see cref="SvgDeferredPaintServer"/> class with the specified parameters.</summary>
    ...
    /// <summary>Gets or sets the SvgDeferredPaintServer(SvgDocument, string, SvgPaintServer).</summary>
    /// <summary>Initializes a new instance of the <see cref="SvgDeferredPaintServer"/> class with the specified parameters.</summary>
    ...
    /// </summary>
    /// <param name="id">&lt;FuncIRI&gt;, &lt;IRI&gt; or &quot;currentColor&quot;.</param>
    /// <param name="fallbackServer">&quot;none&quot;, &quot;currentColor&quot; or <see cref="SvgColorServer"/> server.</param>
    ...
    /// <summary>Gets or sets the EnsureServer(SvgElement).</summary>
    /// <summary>Performs the EnsureServer(SvgElement) operation.</summary>
    public void EnsureServer(SvgElement styleOwner)
    {
        if (!_serverLoaded && styleOwner != null)
        {
            ...
            else
            {
                ...
                else if (!(_fallbackServer is SvgColorServer ||
    ...
    /// <summary>Gets or sets the TryGet.</summary>
    /// <summary>Attempts to perform the TryGet operation.</summary>
