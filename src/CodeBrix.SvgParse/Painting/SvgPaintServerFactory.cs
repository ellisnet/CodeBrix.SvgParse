...
{
    private static readonly SvgColorConverter _colorConverter;
    ...
    public static SvgPaintServer Create(string value, SvgDocument document)
    {
        ...
        // Otherwise try and parse as color
        return new SvgColorServer((SvgColor)_colorConverter.ConvertFrom(colorValue));
    }
    ...
    public override object ConvertTo(ITypeDescriptorContext context, System.Globalization.CultureInfo culture, object value, Type destinationType)
    {
        if (destinationType == typeof(string))
        {
            ...
            var colorServer = value as SvgColorServer;
            if (colorServer != null)
            {
                // Use the SvgColorServer's ToString(), which consults the owning
                // document's EnableEmitNamedColorsOnSerialization flag.
                return colorServer.ToString();
            }
